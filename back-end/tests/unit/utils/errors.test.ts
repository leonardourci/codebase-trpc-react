...
describe('Error Utilities', () => {
    describe('CustomError', () => {
        ...
        it('should create custom error with UNAUTHORIZED status', () => {
            const error = new CustomError('Access denied', StatusCodes.UNAUTHORIZED)
        ...
        it('should create custom error with INTERNAL status', () => {
            const error = new CustomError('Internal server error', StatusCodes.INTERNAL_SERVER_ERROR)
        ...
        it('should create custom error with CONFLICT status', () => {
            const error = new CustomError('Resource already exists', StatusCodes.CONFLICT)
        ...
        it('should create custom error with PRECONDITION_FAILED status', () => {
            const error = new CustomError('Precondition failed', StatusCodes.PRECONDITION_FAILED)

            expect(error.message).toBe('Precondition failed')
            expect(error.statusCode).toBe(StatusCodes.PRECONDITION_FAILED)
        })

        it('should create custom error with NOT_ACCEPTABLE status', () => {
            const error = new CustomError('Not acceptable', StatusCodes.NOT_ACCEPTABLE)
        ...
        it('should create custom error with empty message', () => {
            const error = new CustomError('', StatusCodes.BAD_REQUEST)
        ...
        it('should inherit from Error class', () => {
            const error = new CustomError('Test', StatusCodes.INTERNAL_SERVER_ERROR)
        ...
        it('should have correct stack trace', () => {
            const error = new CustomError('Test error', StatusCodes.INTERNAL_SERVER_ERROR)
    ...
    describe('ZodValidationError', () => {
        it('should create validation error from ZodError with single issue', () => {
            ...
            expect(error.statusCode).toBe(StatusCodes.UNPROCESSABLE)
    ...
    describe('Error inheritance and behavior', () => {
        it('should allow instanceof checks for all error types', () => {
            const customError = new CustomError('Test', StatusCodes.BAD_REQUEST)
        ...
        it('should be throwable and catchable', () => {
            expect(() => {
                throw new CustomError('Test error', StatusCodes.INTERNAL_SERVER_ERROR)
            }).toThrow(CustomError)
        ...
        it('should preserve error properties when caught', () => {
            try {
                throw new CustomError('Test message', StatusCodes.NOT_FOUND)
            } catch (error) {
                expect(error).toBeInstanceOf(CustomError)
                expect((error as CustomError).message).toBe('Test message')
                expect((error as CustomError).statusCode).toBe(StatusCodes.NOT_FOUND)
            }
        })
    })
})