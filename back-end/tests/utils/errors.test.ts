...
const createMockZodError = (issues: Partial<ZodError['issues'][0]>[]): ZodError => {
	const fullIssues: ZodError['issues'] = issues.map(
		(issue) =>
			({
				code: 'custom',
				path: [],
...
describe('Error Utilities', () => {
	...
	describe('ZodValidationError', () => {
		it('should create validation error from ZodError with single issue', () => {
			const mockZodError = createMockZodError([{ message: 'Field is required', path: ['field'], code: 'invalid_type' }])
		...
		it('should create validation error from ZodError with multiple issues', () => {
			...
			expect(error.message).toBe('Validation failed')
			expect(error.statusCode).toBe(EStatusCodes.UNPROCESSABLE)
			expect(error.messages).toEqual(['field1: Field is required', 'field2: Must be a string', 'field3: Must be at least 3 characters'])
		})
		...
		it('should inherit from CustomError', () => {
			const mockZodError = createMockZodError([{ message: 'Test', path: [], code: 'custom' }])
		...
		it('should extract messages from complex ZodError issues', () => {
			...
			expect(error.messages).toEqual(['user,email: Invalid email format', 'user,password: Password must be at least 8 characters'])
		})

		it('should handle ZodError with nested path structures', () => {
			const mockZodError = createMockZodError([{ message: 'Required field missing', path: ['data', 'user', 'profile', 'name'], code: 'invalid_type' }])
		...
		it('should always use UNPROCESSABLE status code', () => {
			const mockZodError = createMockZodError([{ message: 'Any validation error', path: [], code: 'custom' }])
		...
		it('should always use "Validation failed" message', () => {
			const mockZodError = createMockZodError([{ message: 'Specific field error', path: ['field'], code: 'invalid_type' }])
