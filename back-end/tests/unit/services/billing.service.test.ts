...
import type { UpdateUserBillingInput } from '../../../src/types/billing'
...
describe('Billing Service', () => {
    ...
    describe('registerUserBilling', () => {
        ...
        it('should throw error when user is not found', async () => {
            ...
            await expect(registerUserBilling(billingInput)).rejects.toThrow(
                `User with email "${billingInput.userEmail}" not found`
            )
    ...
    describe('updateBillingOnSubscriptionDeleted', () => {
        it('should update billing status to canceled and set expiration to now', async () => {
            ...
            expect(mockBillingRepository.updateBillingById).toHaveBeenCalledWith(
                {
                    id: mockBilling.id,
                    updates: { status: 'canceled', expiresAt: expect.any(Date) }
                },
                expect.anything()
            )
        })
